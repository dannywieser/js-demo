...
const mountComponent = (pathname = '/'): ReactWrapper<IDemoPageProps, IDemoPageState, DemoPageBase> => {
...
it('should toggle the editOpen flag on invocation of the toggleEdit function', () => {
  const wrapper = mountComponent();
  expect(wrapper.state().editOpen).toBeFalsy();
  wrapper.instance().toggleEdit();
  expect(wrapper.state().editOpen).toBeTruthy();
});
it('should toggle the navOpen flag on invocation of the toggleMenu function', () => {
  const wrapper = mountComponent();
  expect(wrapper.state().navOpen).toBeFalsy();
  wrapper.instance().toggleMenu();
  expect(wrapper.state().navOpen).toBeTruthy();
});
